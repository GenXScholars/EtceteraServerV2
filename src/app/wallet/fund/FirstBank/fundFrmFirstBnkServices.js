...
async function initiateFirstBnk(passedBodyParams) {
  ...
  if(passedBodyParams){
    ...
    const { accountnumber, currency, amount, email, DateOfBirth, bvn,  phonenumber, firstname, lastname, IP  } = passedBodyParams;
      return await rave.initiatePayment({
            "PBFPubKey": PubKEY,
            "accountbank": "011",// get the bank code from the bank list endpoint.
...
async function validateFirstBnk(passedBodyParams){
 // function to redirect user to new window to open GT bank services
}

async function verifyFirstBnk(results, req){
